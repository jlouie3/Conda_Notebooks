...
*/
DECLARE @TotalTransactions DECIMAL(19,9) = (SELECT COUNT(DISTINCT TransactionId) FROM ProductTransactionTrain);
...
,JointSupports AS (
	SELECT 
		...
		,COUNT(*) / @TotalTransactions AS Support
	FROM TwoSets ts
	INNER JOIN ProductTransactionTrain t1
	ON t1.ItemId = ts.ItemId1
	INNER JOIN ProductTransactionTrain t2

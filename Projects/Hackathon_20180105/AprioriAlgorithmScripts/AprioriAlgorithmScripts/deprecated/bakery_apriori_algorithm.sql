...
*/
DECLARE @TotalTransactions DECIMAL(19,9) = (SELECT COUNT(DISTINCT TransactionId) FROM BakeryTransaction);
DECLARE @MIN_SUPPORT DECIMAL(19,9) = .002;		-- Used to filter out unpopular items or outliers and save on computation
DECLARE @MIN_CONFIDENCE DECIMAL(19,9) = .04;	-- Used to filter out any rules that are not compelling enough

WITH Supports AS (
	SELECT 
		ItemId 
		,COUNT(*) / @TotalTransactions AS Support
	FROM BakeryTransaction
	GROUP BY ItemId
)
,TwoSets AS (
	SELECT 
...
,JointSupports AS (
	SELECT 
		...
		,COUNT(*) / @TotalTransactions AS Support
	FROM TwoSets ts
	INNER JOIN BakeryTransaction t1
	ON t1.ItemId = ts.ItemId1
	INNER JOIN BakeryTransaction t2
...
SELECT 
	c.ItemId1
	,c.ItemId2
	,bi1.ItemName AS Item1Name
	,bi2.ItemName AS Item2Name
	,c.Confidence
	,c.JointSupport
	,c.Item1Support
	,c.Confidence / c.Item1Support AS Lift
FROM Confidence c
INNER JOIN BakeryItem bi1
ON bi1.ItemId = c.ItemId1
INNER JOIN BakeryItem bi2
ON bi2.ItemId = c.ItemId2
